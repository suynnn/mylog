document.addEventListener('DOMContentLoaded', function () {
    ...
    const tags = hiddenTagsInput ? hiddenTagsInput.slice(1, -1).split(',').map(tag => tag.trim()).filter(tag => tag) : [];
    ...
    selectSeriesBtn.addEventListener('click', function () {
        ...
        if (selectedSeriesRadio) {
            ...
            document.getElementById('seriesInput').value = selectedSeriesRadio.value;

            seriesRegisterBtn.textContent = selectedSeriesName;
            seriesArea.classList.add('d-none'); // Hide series area
        } else {
            alert('시리즈를 선택해주세요.');
        }
